...
class GuestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $invite->id_personne = $personne->id_personne;
            $invite->invite_par = $user->id_personne;
            $invite->commentaire = $request->commentaire;
            $invite->date_inscription = now();
            $invite->actif = true;
            $invite->save();

            $this->logAction($user->id_personne, 'create_guest', "Création d'un nouvel invité: {$personne->prenom} {$personne->nom}", $request);
            ...
            Log::error('Erreur lors de la création d\'invité: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $personne = \App\Models\Personne::find($id);
        if (!$personne) {
            return response()->json(['success' => false, 'message' => 'Invité non trouvé'], 404);
        }
        ...
        try {
            ...
            $this->logAction($user->id_personne, 'delete_guest', "Suppression de l'invité: {$personne->prenom} {$personne->nom}", $request);
            ...
            Log::error('Erreur lors de la suppression d\'invité: ' . $e->getMessage());
            return response()->json([
                'success' => false,
