...
Route::middleware('auth:sanctum')->group(function () {
    // Recherche de résidents (pour popup création visite)
    Route::get('/residents/search', function (Request $request) {
        $q = $request->query('q', '');
        $residents = DB::table('personne')
            ->join('resident', 'personne.id_personne', '=', 'resident.id_personne')
            ->where(function($query) use ($q) {
                $query->where('personne.nom', 'like', "%$q%")
                    ->orWhere('personne.prenom', 'like', "%$q%")
                    ->orWhere('personne.email', 'like', "%$q%")
                ;
            })
            ->select('personne.id_personne', 'personne.nom', 'personne.prenom', 'personne.email', 'personne.numero_telephone')
            ->limit(15)
            ->get();
        return response()->json(['residents' => $residents]);
    });

    // Recherche de visiteurs (tous sauf résidents, pour popup création visite)
    Route::get('/visitors/search', function (Request $request) {
        $q = $request->query('q', '');
        $residentsIds = DB::table('resident')->pluck('id_personne');
        $visitors = DB::table('personne')
            ->whereNotIn('id_personne', $residentsIds)
            ->where(function($query) use ($q) {
            ...
            ->select('id_personne', 'nom', 'prenom', 'email', 'numero_telephone')
            ->limit(15)
            ->get();
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        Route::get('/visites', function () {
            ...
            if ($role === 'admin' || $role === 'gardien') {
                // Admins et gardiens voient toutes les visites
                $visites = DB::table('visite')
                    ->orderBy('date_visite_start', 'desc')
                    ->get();
            } else {
                // Sinon, seulement les visites où il est visiteur ou invité
                $visites = DB::table('visite')
