<template>
  <div class="p-4 sm:p-6">
    ...
    <p class="mb-4 text-gray-500 dark:text-gray-400">{{ $t('adminLogs.subtitle') }}</p>
    ...
    <div class="mb-4 flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
      <input
        ...
        class="w-full md:w-80 px-3 py-2 border border-gray-200 dark:border-gray-700 rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
      />
      <select v-model="actionFilter" class="px-2 py-2 border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-800 w-full md:w-auto">
        <option value="">{{ $t('adminLogs.allActions') }}</option>
        <option v-for="action in uniqueActions" :key="action" :value="action">{{ action }}</option>
      </select>
      <select v-model="userFilter" class="px-2 py-2 border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-800 w-full md:w-auto">
      ...
      <div class="flex items-center gap-2 self-end md:self-auto w-full md:w-auto">
        <input type="date" v-model="deleteBeforeDate" class="px-2 py-2 border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-800 text-xs h-[40px] md:h-auto w-full md:w-auto" style="height: 40px; min-width: 0;" />
    ...
    <div v-if="loading" class="text-center py-8 text-gray-400 dark:text-gray-500">{{ $t('adminLogs.loading') }}</div>
    <div v-else-if="error" class="text-red-500 text-center py-8">{{ error }}</div>
    <div v-else>
      <div v-if="filteredLogs.length === 0" class="text-gray-400 dark:text-gray-500 italic">{{ $t('adminLogs.noLogs') }}</div>
      <ul class="divide-y divide-gray-100">
        <li v-for="log in filteredLogs" :key="log.id" class="py-2 flex flex-col md:flex-row md:items-center gap-1 md:gap-4">
          <span class="text-xs text-gray-400 dark:text-gray-500 w-32">{{ formatDate(log.created_at) }}</span>
          ...
          <span class="text-xs text-gray-500 dark:text-gray-400">{{ log.user_email ?? $t('adminLogs.system') }}</span>
