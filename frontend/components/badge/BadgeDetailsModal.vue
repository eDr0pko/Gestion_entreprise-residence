<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black bg-opacity-50">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
      <!-- En-tête de la modal -->
      <div class="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
        <div class="flex items-center space-x-2 sm:space-x-3">
          <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white bg-opacity-20 flex items-center justify-center font-bold text-sm sm:text-base">
            {{ badge.numero }}
          </div>
          <div>
        ...
        <button
          @click="$emit('close')"
          class="p-1.5 sm:p-2 hover:bg-white hover:bg-opacity-20 rounded-lg transition-colors"
        >
      ...
      <div class="overflow-y-auto max-h-[calc(95vh-180px)] sm:max-h-[calc(90vh-180px)]">
        ...
        <div class="p-3 sm:p-4 border-b border-gray-200">
          <h4 class="text-sm font-semibold text-gray-900 mb-3">{{ t('badges.details.information') }}</h4>
          <div class="space-y-4">
            <!-- Numéro et statut -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">{{ t('badges.fields.number') }}</label>
                <div class="bg-gray-50 rounded-lg px-3 py-2 text-sm font-medium text-gray-900">
                  #{{ badge.numero }}
                </div>
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">{{ t('badges.fields.status') }}</label>
            ...
            <!-- Utilisateur assigné -->
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">{{ t('badges.fields.assignedTo') }}</label>
              ...
              <div v-if="editForm.selectedUser" class="mt-2 p-2 bg-blue-50 rounded-lg">
                <div class="flex items-center justify-between">
                  <div>
                    <div class="text-sm font-medium text-gray-900">{{ editForm.selectedUser.nom_complet }}</div>
                    <div class="text-xs text-gray-500">{{ editForm.selectedUser.email }}</div>
                  </div>
            ...
            <!-- Droits d'accès -->
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">{{ t('badges.fields.rights') }}</label>
              <input
            ...
            <!-- Commentaire -->
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">{{ t('badges.fields.comment') }}</label>
              <textarea
        ...
        <div class="p-3 sm:p-4">
          <h4 class="text-sm font-semibold text-gray-900 mb-3">{{ t('badges.activity.title') }}</h4>
          ...
          <div v-else-if="activities.length === 0" class="text-center py-4">
            <ClockIcon class="w-6 h-6 mx-auto text-gray-400" />
            <p class="text-xs text-gray-500 mt-2">{{ t('badges.activity.noActivity') }}</p>
          </div>
          ...
          <div v-else class="space-y-2">
            <div
              ...
              class="flex items-start space-x-2 p-2 rounded-lg hover:bg-gray-50"
            >
              <div class="flex-shrink-0 mt-1">
                <div class="w-2 h-2 rounded-full bg-blue-500"></div>
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-xs text-gray-900">{{ formatActivityMessage(activity) }}</p>
                <p class="text-xs text-gray-500">{{ formatDate(activity.date_heure || activity.date_action || activity.created_at) }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Footer avec boutons -->
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 p-3 sm:p-4 border-t border-gray-200 bg-gray-50">
        <button
          @click="$emit('close')"
